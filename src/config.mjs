...
export function loadConfig(configPath, options = {}) {
  ...
  const passwordPlaceholders = new Set(["在这里填写你的统一认证密码", "你的统一认证密码", "你的密码"]);
  if (!options.skipCredentialValidation && ((!password || passwordPlaceholders.has(password)) && !iclassLoginName)) {
    throw new Error("请先运行 npm run setup，填写统一认证密码。旧配置也可继续使用 iclassLoginName。");
  }
  if (String(config.mode || "auto") !== "auto") {
    throw new Error("配置 mode 只支持 auto；手动签到请运行 npm run signin。");
  }
  validateNumber(config, "triggerMinutesBeforeClass", 0, 120);
  validateNumber(config, "pollIntervalSeconds", 5, 3600);
  validateNumber(config, "remoteRefreshSeconds", 60, 86400);
  if (config.writeLogs !== undefined && typeof config.writeLogs !== "boolean") {
    throw new Error("配置 writeLogs 必须是 true 或 false。");
  }
...
function validateNumber(config, key, minimum, maximum) {
  if (config[key] === undefined) return;
  if (typeof config[key] !== "number" || !Number.isFinite(config[key]) || config[key] < minimum || config[key] > maximum) {
    throw new Error(`配置 ${key} 必须是 ${minimum} 到 ${maximum} 之间的数字。`);
  }
}
