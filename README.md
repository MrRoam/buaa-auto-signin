...
- **停止后是否会自动恢复：** `stop-windows-background.ps1` 会禁用全部计划任务并结束后台进程，之后不会自行恢复；再次运行 `start-windows-background.ps1` 即可启用。
- **后台进程会被自动拉起吗：** 会。常驻进程在遇到意外错误时会自行记录并退避重试，不再需要额外的定时健康检查任务，因此运行期间不会周期性弹出终端窗口。如果进程真的退出，任务计划程序会按 `RestartCount` 重启，登录时也会自动启动。
